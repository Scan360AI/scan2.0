/**
 * =================================================================
 * SCAN360 - Motore di Rendering del Report v2.2 (Architettura Robusta)
 * File: /report/assets/js/app.js
 * =================================================================
 */

// L'intera logica viene eseguita solo quando la finestra e tutte le risorse
// (incluso Chart.js) sono state completamente caricate.
window.addEventListener('load', () => {

    // --- 1. CONFIGURAZIONE E COSTANTI ---
    const DATA_FILE_PATH = 'data/report_data.json'; // Togliamo lo slash iniziale, sarà relativo alla pagina corrente.
    ...
    // --- 3. FUNZIONI DI INIZIALIZZAZIONE GRAFICI ---
    const initChart = (canvasId, chartConfig) => {
        // Ora Chart è sicuramente definito perché siamo nell'evento 'load'
        const canvas = document.getElementById(canvasId);
        if (!canvas) {
            console.warn(`Canvas con ID "${canvasId}" non trovato.`);
            return;
        }
                    ...
                    callbacks: {
                         label: function(context) {
                             ...
                             if (typeof value === 'number') {
                                 ...
                                 } else if (datasetLabel.includes('/EBITDA') || datasetLabel.includes('(x)')) {
    ...
    // --- 4. FUNZIONI DI RENDERING DEI COMPONENTI ---
    ...
    // ... (le altre funzioni di rendering come renderFooter, renderPrintHeader, etc. rimangono invariate)
     const renderFooter = () => {
    ...
    const renderDashboardPage = (data) => {
        ...
        getValue(data, 'dashboard.kpi_cards', []).forEach(kpiData => {
            ...
            const kpiCardHtml = `<div class="kpi-card-v2 ${kpiData.border_class}"><i class="${kpiData.icon_class} kpi-icon-modern-bg"></i><h4 class="card-title-modern">${kpiData.title}</h4><div class="kpi-value-modern">${kpiData.value}</div><div class="kpi-trend-modern ${kpiData.trend_class}"><i class="${kpiData.trend_icon} trend-icon"></i>${trendValueHtml}<span class="text-muted small ms-2">${kpiData.trend_period}</span></div><p class="kpi-description-modern">${kpiData.description}</p><a href="${kpiData.link_href}" class="btn btn-outline-primary btn-sm btn-pill mt-2 ${kpiData.link_class}">${kpiData.link_text}</a></div>`;
    ...
    const renderSintesiPage = (data) => {
        ...
        summaryContainer.innerHTML = `<h2 class="section-title"><i class="${sintesiData.summary_section.icon} me-2"></i>${sintesiData.summary_section.title}</h2><div class="irp-visual-section"><div class="row align-items-center"><div class="col-md-4 text-center mb-4 mb-md-0"><div class="irp-score-circle ${irp.style_class_circle}"><span class="irp-score-value">${irp.score}</span><span class="irp-score-max">/ 100</span></div><div class="irp-category-text ${irp.style_class_text} mt-2">Rischio ${irp.category_text} <span class="status-badge ${irp.style_class_badge}">${irp.category_letter}</span></div></div><div class="col-md-8"><h4 class="mb-3 fw-bold" style="color: var(--primary);">Valutazione Generale</h4><p class="mb-3">${sintesiData.summary_section.summary_text}</p><div class="other-ratings border-top pt-3 mt-3"><h6 class="card-title-small mb-2">Altri Indicatori:</h6><ul class="list-unstyled mb-0 row gx-3 gy-2">${ratingsHtml}</ul></div></div></div></div>`;
        ...
        profileContainer.innerHTML = `<h2 class="section-title"><i class="${sintesiData.profile_section.icon} me-2"></i>${sintesiData.profile_section.title}</h2><div class="card"><div class="card-body"><div class="row"><div class="col-lg-6"><h6>Anagrafica e Settore</h6><ul class="list-unstyled">${anagraficaHtml}</ul></div><div class="col-lg-6"><h6>Struttura e Personale</h6><ul class="list-unstyled">${strutturaHtml}</ul></div><div class="col-12 mt-3"><h6>Modello di Business</h6>${sintesiData.profile_section.business_model_html}</div></div></div></div>`;
        ...
        for(const key in swot){
            ...
            swotHtml += `<div class="col-md-6 col-lg-3 mb-4"><div class="dashboard-card h-100"><div class="card-header d-flex align-items-center"><i class="${swot[key].icon} me-2"></i> ${swot[key].title}</div><div class="card-body"><ul>${pointsHtml}</ul></div></div></div>`;
        }
        swotHtml += '</div>';
        ...
        cciiContainer.innerHTML = `<h2 class="section-title"><i class="${ccii.icon} me-2"></i>${ccii.title}</h2><div class="alert ${ccii.status_class} d-flex align-items-center"><i class="${ccii.status_icon} fa-2x me-3"></i><div><h6 class="mb-0">${ccii.status_text}</h6>${ccii.description_html}</div></div>`;
    };
    ...
    const main = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status} - Impossibile trovare il file ${DATA_FILE_PATH}`);
            }
            const reportData = await response.json();

            // Rimuovi l'indicatore di caricamento solo dopo il fetch corretto
            document.getElementById('loading-indicator')?.remove();
            
            // Popola le parti comuni
            ...
            const wrapper = document.getElementById(`${PAGE_ID}-content-wrapper`) || document.getElementById('report-sections-wrapper');
            ...
            switch (PAGE_ID) {
                case 'dashboard':
                    ...
                    for (const chartKey in reportData.dashboard.charts) {
                        const chart = reportData.dashboard.charts[chartKey];
                        initChart(`${chartKey}Chart`, chart);
                    }
                    break;
                case 'parte1_sintesi':
                    renderSintesiPage(reportData);
                    // Nessun grafico da inizializzare in questa pagina per ora
                    break;
            }
        } catch (error) {
            console.error("ERRORE CRITICO:", error);
            const container = document.getElementById('main-report-content') || document.body;
            container.innerHTML = `<div class="alert alert-danger p-4"><h4><i class="fas fa-exclamation-triangle me-2"></i>Errore nel Caricamento del Report</h4><p>Non è stato possibile caricare o processare il file di dati <code>${DATA_FILE_PATH}</code>. Assicurati che il file esista, sia accessibile e non contenga errori di sintassi JSON.</p><pre class="small bg-light p-2 rounded">${error.message}</pre></div>`;
            document.getElementById('loading-indicator')?.remove();
        }
    };
